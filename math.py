...
def sectorarea():
    ...
    if angle >= 361:
        print("Angle is not possible")
    return
    surface_area = (pi*radius**2) * (angle/360)
...
while RUN_AGAIN in ('y', 'Y'):
    ...
    if SELECT in AREA_CIRCLE_LS:
        areacircle()
    elif SELECT in DEGREE_TO_RADIAN_LS:
        degree2radian()
    elif SELECT in RADIAN_TO_DEGREE_LS:
        radian2degree()
    elif SELECT in AREA_TRAPEZOID_LS:
        areatrapezoid()
    elif SELECT in AREA_PARALLELOGRAM_LS:
        areaparallelogram()
    elif SELECT in AREA_TRIANGLE_LS:
        areatriangle()
    elif SELECT in SA_CYLINDER_LS:
        sacylinder()
    elif SELECT in VOLUME_CYLINDER_LS:
        volumecylinder()
    elif SELECT in DISCRIMINANT_LS:
        discriminant()
    elif SELECT in SA_SPHERE_LS:
        sasphere()
    elif SELECT in VOLUME_SPHERE_LS:
        spherevolume()
    elif SELECT in QUADRATIC_FORMULA_LS:
        quadraticformula()
    elif SELECT in CIRCUMFERENCE_LS:
        circumference()
    elif SELECT in LENGTH_ARC_LS:
        arclength()
    elif SELECT in AREA_SECTOR_LS:
        sectorarea()
    elif SELECT in PROGRAM_LS:
        programs()
        LIST = True
    else:
        print('That is not a program            For Program List Type "p"')
    LIST = True
